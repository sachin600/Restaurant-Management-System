...
def total_bills():
        ...
        milk_q = milk_qty.get()
        monster_q = monster_qty.get()
        redbull_q = redbull_qty.get()

        # ============= Foods Item quantity ======================
        roti_q = roti_qty.get()
        dal_fry_q = dal_fry_qty.get()
        ...
        if monster_var.get() == 0:
                monster_q = 0
        elif monster_var.get() == 1 and monster_qty.get() == "":
                messagebox.showerror("error","please fill the monster quantity")
                monster_q = 0
        ...
        if dal_fry_var.get() == 0:
                dal_fry_q = 0
        elif dal_fry_var.get() == 1 and dal_fry_qty.get() == "":
                messagebox.showerror("error","please fill the Dal fry quantity")
                dal_fry_q = 0
        ...
        total_monster_price = monster_price * int(monster_q)
        total_redbull_price = redbull_price * int(redbull_q)

        # ============ Total Drinks cost ===================
        total_drinks_cost = total_lassi_price + total_coffee_price + total_tea_price + total_juice_price + total_shakes_price + total_milk_price + total_monster_price + total_redbull_price
        ...
        total_dal_fry_price = dal_fry_price * int(dal_fry_q)
        ...
        total_rice_price = rice_price * int(rice_q)
         ...
         # ============ Total Foods cost ===================
        total_foods_cost = total_roti_price + total_dal_fry_price + total_mutter_panner_price + total_paratha_price + total_mix_veg_price + total_omelete_price + total_veg_biryani_price + total_rice_price
...
def monster_chk():
        if monster_var.get() == 1:
                monster_qty['state'] = "normal"
                monster_qty['bg'] = '#248aa2'
                monster_qty['fg'] = "white"
        else:
                monster_qty['state'] = "disabled"
...
def dal_fry_chk():
        if dal_fry_var.get() == 1:
                dal_fry_qty['state'] = "normal"
                dal_fry_qty['bg'] = '#248aa2'
                dal_fry_qty['fg'] = "white"
        else:
               dal_fry_qty['state'] = "disabled"
...
l1 = Label(frame,text="HOTEL MANAGEMENT SYSTEM",font=('roboto',30,'bold'),bg='#248aa2',fg="#ffffff")
l1.place(x=10,y=4)
...
monster_var = IntVar()
monster = Checkbutton(drinks,text="monster",variable=monster_var,font=('verdana',8,'bold'),onvalue=1,offvalue=0,command=monster_chk)
monster.place(x=2,y=132)
monster_qty = Entry(drinks,width=7,borderwidth=4,relief=SUNKEN,state="disabled")
monster_qty.place(x=74,y=132)
...
dal_fry_var = IntVar()
dal_fry = Checkbutton(foods,text="Dal fry",variable=dal_fry_var,font=('verdana',8,'bold'),command=dal_fry_chk)
dal_fry.place(x=2,y=22)
dal_fry_qty = Entry(foods,width=15,borderwidth=4,relief=SUNKEN,state="disabled")
dal_fry_qty.place(x=140,y=22)
